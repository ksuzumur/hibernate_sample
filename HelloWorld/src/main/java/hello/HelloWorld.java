...
public class HelloWorld {
    ...
    public void runHibernate() {
        ...
        List messages = secondSession.createQuery(
        ...
        Message loadedMessage = (Message) thirdSession.get(Message.class,
                message.getId());
        loadedMessage.setText("Greetings Earthling");
        loadedMessage.setNextMessage(new Message(
        ...
        messages = fourthSession.createQuery(
                "from Message m order by m.text asc").list();
        System.out.println(messages.size() + " message(s) found:");
        for (Iterator iter = messages.iterator(); iter.hasNext();) {
            Message loadedMsg = (Message) iter.next();
